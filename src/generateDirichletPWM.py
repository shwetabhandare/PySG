...
def getFirstCharacterToAdd(lineCount):
	if lineCount == 0:
		lineToAdd = "A:"
	elif lineCount == 1:
		lineToAdd = "C:"
...
def writePwmFile(pwm_array, pwmFile, scale_factor):
	f = open(pwmFile, 'w');
	pwmFileName = os.path.splitext(os.path.basename(pwmFile))[0]
	print "PWM FILE: ", pwmFileName;
	titleLine = "Gene: " + pwmFileName + ", Scale Factor: " + str(scale_factor) + "\n"
	f.write(titleLine);
	lineCount = 0;
	for pwmLine in pwm_array:
		lineToAdd = getFirstCharacterToAdd(lineCount);
